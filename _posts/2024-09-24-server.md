...
알고리즘 측면에서는 두 코드가 같은데, 시간이 약 3배 이상 차이가 나는 이유는? 바로 **캐시**때문이다. 첫 번째 코드에서는 순차적으로 접근하기 때문에 캐시 히트가 발생하고, 두 번째 코드에서는 첫 번째, 만 번 뒤, 만 번 뒤... 이렇게 띄엄띄엄 접근하기 때문에 캐시 히트가 일어나지 않는다. 
