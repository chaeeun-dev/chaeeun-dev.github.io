---
title: "[Github] Jekyll 블로그 잔디 안 심어지는 문제"
...
깃허브 블로그에 매일 til을 작성하면서 1일 1잔디를 심으려고 했지만, 깃허브 블로그에 커밋을 하면 잔디가 안 심어지는 문제가 발생했다(다른 레파지토리에서 커밋하면 정상적으로 반영 됨). 메일과 아이디 문제는 아니었고 fork 했을 때 branch 설정 때문인 것 같았다.
