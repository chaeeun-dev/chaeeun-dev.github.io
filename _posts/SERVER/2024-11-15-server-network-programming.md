...
## Selecet, WSAEventSelect(24.11.22, 12.05)
...
### WSAEvent의 단점
WSA나 IOCP 등은 모두 윈도우 환경에서만 돌아가기 때문에 모바일 게임에서의 구현이 불가능하다. 참고로 모바일 게임은 주로 리눅스를 사용한다.

---

### Server vs Client
- 서버는 환경을 타지 않는다. (모바일, PC 환경 모두 상관 없음)
- 클라는 언리얼, 유니티를 사용하기 때문에 네트워크 C++ 코드를 붙여서 사용해야 한다. 모바일, PC 환경에서의 빌드가 이루어져야 한다.
...
### 블로킹, 논블로킹 / 동기, 비동기
- 비동기 : 요청은 지금 했지만 나중에 여유 있을 때 실행해도 된다.
- 비동기는 멀티쓰레드인가? X. 
- 나누는 기준
	- 동기 vs 비동기 - 요청했을 때 바로 실행이 되는지? 아닌지?
	- 블로킹 vs 논블로킹 - 함수를 호출하면 대기하는지? 바로 완료되는지?


![image](https://github.com/user-attachments/assets/6f67fcd9-c4a9-4847-b1c6-02c81c3abb8a)

- Sync-Blocking
	- 게임에서는 모두가 일해야하는데 다른 일을 할 수 없어서 문제
- Sync-NonBlocking
	- 장점 : 바로바로 일을 수행할 수 있음.
	- 단점 : 필요가 없어도 계속 반환해야 함. (그래서 Select, EventSelect로 우회하는 방식을 사용했음)
- Async-Blocking
	- 블로킹을 애초에 안 할거였으면 비동기는 의미가 없기에 이건 필요 없음!
- Async-NonBlocking
	- Overlapped 모델

### WSARecev, WSASend
- 개념 - 낚시대를 던지고 물고기가 물 때까지 기다리는 것처럼, 나중에라도 누가 데이터를 보내니 대기를 걸어 놓는다.
- 기억할 것 - Event를 통해서 완려가 된 걸 감지해서 연결하는 방식이다.
- Select vs Overlapped
	- Select - 현재 상황에서 받을 수 있을 때 Recieve함.
	- Overlapped - Receive를 먼저 걸고 감지 후 처리함.

### Overlapped
1. Event 이용해 탐지하는 방법 [소스 코드](https://github.com/chaeeun-dev/Server/blob/main/Server/05.Overlapped(event).cpp)
2. Callback 이용해 탐지하는 방법(completion Routine) [소스 코드](https://github.com/chaeeun-dev/Server/blob/main/Server/05.Overlapped(callback).cpp) (Session 구조체에 WSAOVERLAPPED를 무조건 처음으로 위치시켜야 함.)

- Reactor Pattern - Select 모델처럼 소켓 먼저 확인 후 Receive, Send
- Proactor Pattern - Overlapped 모델처럼 일단 Receive 걸어두고 시작, 이벤트가 완료됐을 때 나중에 알려줌.

**기억할 것은 Receive를 먼저 걸어둔다는 것!** 지금은 아리까리해도 IOCP 강의 때 이해가 잘 될 것이라고 한다.
