...
_deltaTime = (currentCount - _prevCount) / static_cast<float>(_frequency);은 (현 시점의 카운터 값 - 이전 시점의 카운터 값) == 경과된 카운트, 경과된 카운트 / 초당 카운터가 돌아가는 수 == 프레임이 시작된 이후 경과된 시간(deltaTime)
