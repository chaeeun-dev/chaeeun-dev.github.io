...
Component class를 생성해서 GameObject에 붙일 수 있게 구현하고, Component를 상속받는 SpriteRenderer를 생성해 sprite를 화면에 그릴 수 있는 부품을 만들어준다.
...
### Flipbook
애니메이션을 나타내는 플립북을 생성하고, ResourceManager에 추가한다.
...
### 레이어
앞에서 그리는 순서에 따라 캐릭터가 가려질 수도 있다고 했는데, 레이어 순서를 만들면 이를 해결할 수 있다.

Enums에 레이어 타입을 생성해준다. 배경, 객체, UI 등..
...
그 다음 Actor class에서 객체들이 Layer로 관리될 수 있도록 SetLayer - GetLayer 함수를 생성한다.

DevScene에서 vector<Actor*> _actors; 이렇게 관리했던 Actor들을 vector<Actor*> _actors[LAYER_MAXCOUNT]; 이렇게 레이어로 나눠 그릴 수 있도록 인덱스를 생성한다.

인덱스만 생성해줘도 그리는 순서를 쉽게 설정할 수 있음!
